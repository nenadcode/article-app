          ...
          <v-flex xs12>
            <v-form v-if="userInfo">
              ...
              <v-card-actions>
                ...
                @click.native="saveAccountInfo">Save</v-btn>
              </v-card-actions>
            </v-form>
          </v-flex>
        </v-layout>
...
export default {
  ...
  methods: {
    onChangeFirstName (firstName) {
      this.changeAccountInfo('first-name', firstName)
    },
    onChangeLastName (lastName) {
      this.changeAccountInfo('last-name', lastName)
    },
    onChangeEmail (email) {
      this.changeAccountInfo('email', email)
    },
    changeAccountInfo (property, value) {
      this.$emit('change-account-info', { property, value })
    },
    saveAccountInfo () {
